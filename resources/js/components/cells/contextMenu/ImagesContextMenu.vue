...
const completeSelectedCellsTask = async () => {
    ...
    // Обновляем дилеи на актуальные: для затронутых батчей перезагружаем диапазон
    try {
        const affectedBatchKeys = new Set(finalized.map(it => `${it.project_id}:${it.batch_id}`))
        const dates = allDatesInThreeMonths.value || []
        if (affectedBatchKeys.size && dates.length >= 2) {
            const startDate = (() => {
            ...
            await Promise.all(Array.from(affectedBatchKeys).map(async (bk) => {
                const [pId, bId] = bk.split(':').map(Number)
...
const clearSelectedCellsColor = async () => {
    ...
    for (const key of selectedKeys) {
        ...
        if (cell && delayStatusId && cell.statusId === delayStatusId) {
            ...
            // задачи удаляем по id, если есть; иначе по составному ключу
            const byId = (cell && typeof cell.id !== 'undefined') ? { id: Number(cell.id) } : composite
            taskItems.push(byId)
        }
    }

    // Если в выделении есть Delay — удаляем только Delay (визуально), задачи не трогаем
    if (delayItems.length > 0) {
        await store.dispatch('coloredCells/deleteLocalDelays', { items: delayItems })
